...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if (min_priority_proc == 0 || p->priority < min_priority_proc->priority)
      	min_priority_proc = p;
     }
    ...
    if (min_priority_proc != 0) {
      ...
      if (p->pid>4)
	      cprintf("Running process %d with priority %d, run count %d\n", p->pid, p->priority, p->count);	
...
int set_proc_priority(int id, int priority) {
	struct proc * p;
